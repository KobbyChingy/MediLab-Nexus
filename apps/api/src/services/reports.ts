...
function calculateAge(dateOfBirth: Date | null | undefined, referenceDate: Date) {
  if (!dateOfBirth) {
    return "Not recorded";
  }
...
async function buildReportBundle(prisma: PrismaClient, reportId: string) {
  const [facility, report] = await Promise.all([
    prisma.facility.findFirst({ orderBy: { createdAt: "asc" } }),
  ...
  return {
    facility: toFacilityProfile(facility),
    report,
    imagePaths,
    fileName,
...
async function buildReceiptBundle(prisma: PrismaClient, paymentId: string) {
  const [facility, payment] = await Promise.all([
    prisma.facility.findFirst({ orderBy: { createdAt: "asc" } }),
    prisma.paymentRecord.findUniqueOrThrow({
      where: { id: paymentId },
      include: {
        invoice: {
          include: {
...
async function buildInvoiceBundle(prisma: PrismaClient, invoiceId: string) {
  const [facility, invoice] = await Promise.all([
    prisma.facility.findFirst({ orderBy: { createdAt: "asc" } }),
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .brand-copy h1 { font-size: 24px; text-transform: uppercase; letter-spacing: 0.03em; }
      .brand-copy h2 { font-size: 14px; text-transform: uppercase; text-decoration: underline; margin-top: 6px; }
      ...
      .section h3 { margin: 0 0 8px; font-size: 16px; text-transform: uppercase; text-decoration: underline; }
  ...
  <body>
    ...
    <article class="sheet">
      ...
      <section class="section"><h3>History</h3><div class="body-copy">${blockify(history)}</div></section>
      <section class="section"><h3>Description</h3><div class="body-copy">${blockify(description)}</div></section>
      ${impression ? `<section class="section"><h3>Impression</h3><div class="body-copy">${blockify(impression)}</div></section>` : ""}
...
export async function ensureReportPdf(prisma: PrismaClient, reportId: string) {
  ...
  const patientAge = calculateAge(bundle.report.patient.dateOfBirth, bundle.report.createdAt);
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    if (bundle.facility.location) {
      doc.moveDown(0.15).fontSize(9).fillColor("#5d6d67").text(bundle.facility.location);
    }
    if (bundle.facility.phone || bundle.facility.email) {
      doc
        ...
        .fontSize(9)
        .fillColor("#5d6d67")
        .text([bundle.facility.phone, bundle.facility.email].filter(Boolean).join(" / "));
    }
    doc.moveDown(0.45);
    doc.fontSize(20).fillColor("#14231f").text(bundle.report.title, { align: "center" });
    doc.moveDown(0.15);
    doc.fontSize(12).fillColor("#0d5f58").text(orderedItems, { align: "center", underline: true });
    doc.moveDown(1);
    ...
    doc
      .fontSize(8)
      .fillColor("#5d6d67")
      .text("Reported by", 42, signoffTop + 22, { width: 203, align: "center" });
    doc
      .fontSize(10)
      .fillColor("#14231f")
    ...
    doc
      .fontSize(9)
      .fillColor("#5d6d67")
      .text(bundle.facility.footerMessage || "Preserve the Patient Trace Code on all printed copies.", {
        align: "center",
      });
...
export async function renderPrintableReceiptHtml(
  ...
  const patientGender = payment.invoice.patient.gender?.trim() || "Not recorded";
...
<html lang="en">
  <head>
    ...
    <style>
      :root { color-scheme: light; font-family: "Segoe UI", Arial, sans-serif; color: #16304a; background: #eef5ff; }
      * { box-sizing: border-box; }
      ...
      .hero h1 { margin-top: 8px; font-size: 28px; }
      .contact { margin-top: 8px; opacity: 0.92; font-size: 14px; }
      ...
      .value { margin-top: 6px; font-size: 17px; font-weight: 700; color: #10233d; }
      .summary-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 14px; }
      .footer { padding: 22px 32px; color: #5b6d82; background: #f8fbff; font-size: 13px; display: grid; gap: 6px; }
      .developer-credit { color: #6b7280; font-size: 12px; }
...
<html lang="en">
  ...
  <body>
    <article class="sheet">
      ...
      <section class="section">
        <div class="meta-grid">
          <div class="meta-card"><div class="label">Coverage</div><div class="value">GHc ${(invoice.insuranceCoveredCents / 100).toFixed(2)}</div></div>
...
export async function renderPrintableFinanceAnalyticsHtml(
  ...
  const facility = toFacilityProfile(
    await prisma.facility.findFirst({
      where: actor.facilityId ? { id: actor.facilityId } : undefined,
...
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <title>Operations Report - ${escapeHtml(facility.name)}</title>
    <style>
      :root { color-scheme: light; font-family: "Segoe UI", Arial, sans-serif; color: #16304a; background: #eef5ff; }
      * { box-sizing: border-box; }
      body { margin: 0; padding: 24px; background: linear-gradient(180deg, #edf4ff, #f9fcff); }
      ...
      .contact { margin-top: 8px; opacity: 0.92; font-size: 14px; }
      .section { padding: 22px 32px; border-top: 1px solid rgba(15, 42, 78, 0.08); }
      .section-title { margin: 0 0 14px; font-size: 18px; color: #10233d; }
      ...
      .label { font-size: 12px; text-transform: uppercase; letter-spacing: 0.12em; color: #64748b; }
      .value { margin-top: 6px; font-size: 18px; font-weight: 700; color: #10233d; }
      .footer { padding: 22px 32px; color: #5b6d82; background: #f8fbff; font-size: 13px; display: grid; gap: 6px; }
      .developer-credit { color: #6b7280; font-size: 12px; }
      @media print { body { padding: 0; background: #fff; } .sheet { border: 0; border-radius: 0; box-shadow: none; } }
    </style>
    ...
    <article class="sheet">
      ...
      <section class="section">
        <div class="metric-grid">
          <div class="metric-card"><div class="label">Gross billed</div><div class="value">GHc ${(analytics.summary.grossBilledCents / 100).toFixed(2)}</div></div>
          ...
          <div class="metric-card"><div class="label">Outstanding</div><div class="value">GHc ${(analytics.summary.outstandingCents / 100).toFixed(2)}</div></div>
          <div class="metric-card"><div class="label">Net profit</div><div class="value">GHc ${(analytics.summary.netProfitCents / 100).toFixed(2)}</div></div>
        </div>
      </section>
      <section class="section">
          ...
          ${
            analytics.userPerformance.length > 0
              ? analytics.userPerformance
                  .map(
                    (item) =>
                      `<div class="row-card"><div><strong>${escapeHtml(item.actorName)}</strong><br /><small>Collections GHc ${(item.generatedCents / 100).toFixed(2)} · Expenses GHc ${(item.lossCents / 100).toFixed(2)} · Inventory actions ${item.inventoryActions}</small></div><strong>GHc ${(item.netCents / 100).toFixed(2)}</strong></div>`,
                  )
                  .join("")
              : '<div class="row-card"><div><strong>No staff activity in range</strong></div><strong>GHc 0.00</strong></div>'
          }
        </div>
      </section>
      <section class="section">
        <h2 class="section-title">Collections Trend</h2>
        <div class="row-list">
          ${analytics.monthlyCollections
            .map(
              (month) =>
                `<div class="row-card"><div><strong>${escapeHtml(month.label)}</strong><br /><small>Billed GHc ${(month.billedCents / 100).toFixed(2)} · Collected GHc ${(month.collectedCents / 100).toFixed(2)}</small></div><strong>${month.billedCents > 0 ? `${((month.collectedCents / month.billedCents) * 100).toFixed(1)}%` : "0.0%"}</strong></div>`,
            )
            .join("")}
        </div>
      </section>
      <section class="section">
        <h2 class="section-title">Study Performance</h2>
        <div class="row-list">
          ${
            analytics.studyPerformance.length > 0
              ? analytics.studyPerformance
                  .map(
                    (item) =>
                      `<div class="row-card"><div><strong>${escapeHtml(item.description)}</strong><br /><small>${escapeHtml(item.department)} · ${escapeHtml(item.kind)} · ${item.quantity} study item(s) · ${item.invoicesCount} invoice(s) · Growth ${item.growthRatePercent.toFixed(1)}% · Collected GHc ${(item.collectedCents / 100).toFixed(2)} · Outstanding GHc ${(item.outstandingCents / 100).toFixed(2)}</small></div><strong>GHc ${(item.billedCents / 100).toFixed(2)}</strong></div>`,
                  )
                  .join("")
              : '<div class="row-card"><div><strong>No study activity in range</strong></div><strong>GHc 0.00</strong></div>'
          }
        </div>
      </section>
      <section class="section">
        ...
        <div class="row-list">
          ${
            analytics.expenseCategories
              .map(
                (item) =>
                  `<div class="row-card"><div><strong>${escapeHtml(item.category)}</strong><br /><small>${item.count} expense item(s)</small></div><strong>GHc ${(item.totalCents / 100).toFixed(2)}</strong></div>`,
              )
              .join("") ||
            '<div class="row-card"><div><strong>No expenses recorded</strong></div><strong>GHc 0.00</strong></div>'
          }
        </div>
      </section>
      <footer class="footer">
